...
export default function Comments() {
      ...
      >
        ...
        {numSelected > 0 ? (
          <Tooltip title="Delete">
            <IconButton onClick={deleteCategory} sx={{ color: "#fff" }}>
              <DeleteTwoToneIcon />
            </IconButton>
          </Tooltip>
        ) : (
  ...
  const [rows, setComments] = React.useState([]);
  const [open, setOpen] = React.useState(false);
  const [loading, setLoading] = useState(true);

  function getCommentsData() {
    axios.get(`${process.env.REACT_APP_BACKEND_URL}/comments`).then((response) => {
  ...
  const deleteCategory = () => {
    axios
      .delete(`${process.env.REACT_APP_BACKEND_URL}/comments/${selected}`)
      .then((res) => {
